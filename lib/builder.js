...
class Builder {
  ...
  async transform (transformer, model, scope) {
    ...
    if (
      ...
      data = await transformer.call(transformer, model)
  ...
  async processIncludes (transformer, model, scope) {
    ...
    for (let include of includes) {
      ...
      if (transformer[method] !== undefined) {
        ...
        if (result instanceof types.Collection) {
          let { transformer, model: models } = result
          const promises = models.map(model => this.transform(
            transformer,
            model,
        ...
        else if (result instanceof types.Type) {
